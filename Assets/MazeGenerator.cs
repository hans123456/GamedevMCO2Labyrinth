		...
		for (int i = 0; i <= Maze.GetUpperBound(0); i++)
		{
			for (int j = 0; j <= Maze.GetUpperBound(1); j++)
			{
				if (Maze[i, j] == 1)
				{
					...
					ptype.transform.position = new Vector3((i-startX) * ptype.transform.localScale.x, 0.5f, (j-startZ) * ptype.transform.localScale.y);
					if (brick != null)
					{
				...
				{
					pathMazes.Add(new Vector3((i-startX), j, (j-startZ)));
				}
